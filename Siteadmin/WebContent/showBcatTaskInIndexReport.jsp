...
{  
          ...
          document.index_b_cat.action ="excel.jsp";
          document.forms["index_b_cat"].submit();
}
</script>
</head>
...
%>
<%
String BcatTask = request.getParameter("BcatTask");
...
System.out.println("The BcatTask Tasks is :"+BcatTask);
...
<%
	...
	<form id="index_b_cat" name="index_b_cat" method = "post" >
	<%
         final String exportFileName="B_Category_Task_Report_of_"+user_name+"_Between_"+new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(frmDate))+"_And_"+new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(toTime));
	%>
	<table align="center" width="100%">
			<tr align="center">
				<td align="center">
					<font size="3"><b>B Category Task Report of <%= user_name %> Between <%= new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(frmDate)) %> And <%= new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(toTime)) %></b></font>
				</td>
			</tr>
		</table>
	...
	<table width="100%" border="0" align="center">
		...
		String sql_total ="select * from dotproject.tasks a, dotproject.user_tasks b where a.task_id=b.task_id and user_id='"+user_id+"' and date(task_end_date)>='"+frmDate+"' AND date(task_end_date)<='"+toTime+"' and a.task_percent_complete=100 and hour(timediff(a.task_completion_date_time,a.task_end_date))>24 and hour(timediff(a.task_completion_date_time,a.task_end_date))<=144 ";
