...
try{
	...
	conn = DriverManager.getConnection(MM_dbConn_STRING4,MM_dbConn_USERNAME,MM_dbConn_PASSWORD);
	conn1 = DriverManager.getConnection(MM_dbConn_STRING5,MM_dbConn_USERNAME,MM_dbConn_PASSWORD);
...
<input type="radio" name="limit" value="All" > ALl
	...
	if(!(null==request.getQueryString()))
	{
		...
		sql="select * from db_gps.allconnectedunits where unitid='"+unitid+"' order by concat(TheDate,TheTime) desc limit 1";
	...
	//	String sqlUnitTyp = "select * from db_gps.t_unitmaster where unitid='"+unitid+"'";
		String sqlUnitTyp = "select * from db_gps.t_ftplastdump where UnitID='"+unitid+"'";
		 System.out.println(sqlUnitTyp);
		ResultSet rs= st1.executeQuery(sqlUnitTyp);
		if(rs.next()){
			System.out.println(rs.getString("UnitType"));
			if("IP".equalsIgnoreCase(rs.getString("UnitType"))){
				sqlProcessed="SELECT StoredTime,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom FROM t_ip"+thedate2+"  where Unitid='"+unitid+"'  UNION select StoredTime,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom from t_ip"+thedate2+"Processed where Unitid='"+unitid+"' order by storedtime desc "+limit;	
		}
		else
		{
