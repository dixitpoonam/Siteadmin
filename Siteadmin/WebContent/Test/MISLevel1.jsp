...
//	thedate3=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(thedate2));
	...
	<form name="unitform" action="" method="get" >
		...
		<a href="excelMISLevel1.jsp?data=<%=thedate %>&stamp=<%=stamp %>"  title="Export to Excel">
		...
		while(rs.next())
		{
			...
			<td class="bodyText" align="right" ><div align="right"><%if(rs.getInt("cnt") > 0){%><a href="MISLevel2.jsp?stamp=<%=stamp %>&data=<%=thedate %>&unitid=<%=unitid %>" target="_blank"><%=rs.getInt("cnt")%></a><%}else{%><%=rs.getInt("cnt")%><%} %></div></td>
			</tr>
			<%
		}
		%>
